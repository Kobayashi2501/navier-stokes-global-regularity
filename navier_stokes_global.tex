...
If \( \overline{\mathcal{O}} \) is contractible, then the 1-dimensional persistent homology \( PH_1(\mathcal{O}) \) vanishes in both \v{C}ech and Vietoris–Rips filtrations.
\end{lemma}

\begin{lemma}[Asymptotic Smoothing Excludes Type II/III Blow-Up]
Suppose \( \|u(t)\|_{H^1} \le C(1+t)^{-\delta} \) for some \( \delta > 0 \). Then no singularity with slower divergence rate (Type II/III) can occur.
\end{lemma}

\begin{lemma}[Weak–Strong Uniqueness in \( H^1 \)]
Let \( u,v \in L^\infty(0,T;H^1) \cap L^2(0,T;H^2) \) be two solutions with same initial data and satisfying the energy identity. Then \( u = v \).
\end{lemma}

\begin{lemma}[Uniqueness from Injectivity and Energy Monotonicity]
Let \( u(t), v(t) \in H^1 \) be two solutions with the same initial data, energy decay, and injective orbits. Then \( u(t) = v(t) \) for all \( t \ge 0 \).
\end{lemma}

\begin{proof}[Proof Sketch of Theorem]
Assemble the above lemmas: injectivity and finite length yield a topological arc, which is contractible. This rules out closed loops, implying \( PH_1 = 0 \). A Type I blow-up entails self-similarity and phase space recurrence, contradicting this. For Type II/III, energy dissipation and decay rates enforce uniform regularity. Uniqueness follows from standard weak–strong arguments and injective orbit consistency.
\end{proof}
